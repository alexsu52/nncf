...
class ConstantFolder(torch.fx.Interpreter):
    def __init__(
        ...
        self.gm_params = [name for name, _ in gm.named_parameters()]
    ...
    def run_node(self, node):
        ...
        if any(
            type(self.unknown_value) == type(input_) and self.unknown_value == input_ for input_ in flattened_inputs
        ):
        ...
        if node.op == "get_attr" and node.target not in self.gm_params:
            return self.unknown_value
    ...
    with torch.utils._python_dispatch._disable_current_modes():
        ...
        for node, constant in cf.node_replacements.items():
            if constraint_fn is not None and not constraint_fn(node):
                continue
            replace_node_with_constant(gm, node, constant.contiguous())
...
def constant_graph_tag(gm: torch.fx.GraphModule):
    with torch.utils._python_dispatch._disable_current_modes():
        ...
        for node in gm.graph.nodes:
            if node.op == "get_attr" or node in cf.node_replacements or node in cf.replaced_uses:
