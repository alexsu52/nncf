...
from nncf.openvino.graph.transformations.commands import OVBiasCorrectionCommand, OVBiasInsertionCommand
...
class OVFastBiasCorrectionAlgoBackend(FastBiasCorrectionAlgoBackend):
    ...
    @staticmethod
    def insert_bias_correction_command(node: NNCFNode, bias_value: Tensor) -> OVBiasInsertionCommand:
        return OVCommandCreator.create_command_to_insert_bias(node, bias_value.data)
    ...
    def get_bias_value(node: NNCFNode, nncf_graph: NNCFGraph, model: ov.Model) -> Tensor:
        value = get_bias_value(node, nncf_graph, model)
        if value is None:
            return value
        return Tensor(value)
