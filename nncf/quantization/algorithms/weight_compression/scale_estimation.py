...
class ScaleEstimation:
    ...
    def apply(
        ...
        for wp in track(self._all_weight_params, description="Applying Scale Estimation"):
            ...
            res[weight_name], _ = self.calculate_quantization_params(
    ...
    def calculate_quantization_params(
        ...
        if X_full.shape[1] > subset_size:
            lens = [stat.shape[0] for stat in activations]
        ...
        key = (config.mode, config.num_bits) + q_weights.shape + scale.shape + zp.shape
        if key in ScaleEstimation.compress_decompress_cashe:
            compress_decompress_model = ScaleEstimation.compress_decompress_cashe[key]["compress_decompress_model"]
            compress_model = ScaleEstimation.compress_decompress_cashe[key]["compress_model"]
        else:
            compress_decompress_model = backend_entity.get_compress_decompress_pipeline(
                config, q_weights.shape, scale.shape, zp.shape
            )
            compress_model = backend_entity.get_compress_pipeline(config, q_weights.shape, scale.shape, zp.shape)
            ScaleEstimation.compress_decompress_cashe[key] = {
